...
spec = do
  describe "Compiler tests" $ do
    ...
    it "compiles a function with while loop" $ do
        ...
        let wasmModule = fillWASMModuleFromAST ast
        let expected = WASMModule {types = [TypeSection (FunctionType {funcparams = [ModuleI32], results = [ModuleI32]})], imports = [], functions = [Function {typeIndex = 0, functionCode = CodeSection {locals = [ModuleI32,ModuleI32], codebody = [ModuleConstI32 0,ModuleLocalSet 1,ModuleBlock Nothing [ModuleLoop Nothing [ModuleLocalGet 1,ModuleLocalGet 0,ModuleLt ModuleI32,ModuleEqz ModuleI32,ModuleBrIf 1,ModuleLocalGet 1,ModuleConstI32 1,ModuleAdd ModuleI32,ModuleLocalSet 1,ModuleBr 0]],ModuleLocalGet 1,ModuleReturn]}}], exports = [ExportSection (Export {exportName = "dowhile", exportKind = FunctionExport})], codes = [CodeSection {locals = [ModuleI32,ModuleI32], codebody = [ModuleConstI32 0,ModuleLocalSet 1,ModuleBlock Nothing [ModuleLoop Nothing [ModuleLocalGet 1,ModuleLocalGet 0,ModuleLt ModuleI32,ModuleEqz ModuleI32,ModuleBrIf 1,ModuleLocalGet 1,ModuleConstI32 1,ModuleAdd ModuleI32,ModuleLocalSet 1,ModuleBr 0]],ModuleLocalGet 1,ModuleReturn]}], memories = [], globals = []}
        wasmModule `shouldBe` expected
